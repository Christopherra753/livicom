    ...
    <header className='fixed top-0 w-full z-40 bg-shop-100 px-5'>
      <nav className='flex items-center justify-between max-w-7xl mx-auto'>
        ...
        <ul className='hidden md:flex gap-4'>
          <MenuLink address='/products' label='Productos' />
          <MenuLink address='/about' label='Nosotros' />
          {
              user
                ? <MenuLink address='/profile' label='Perfil' />
                : (
                  <>
                    <MenuLink address='/login' label='Iniciar' />
                    <MenuLink address='/register' label='Registrarse' />
                  </>
                  )
            }
          {
                user?.rol === 'admin' && <MenuLink address='/dashboard' label='Dashboard' />
            }
        </ul>

        <ul className={`absolute z-10 top-0 right-0 w-full bg-black/90 h-screen ${showMenu ? 'translate-x-0' : 'translate-x-full'} duration-300`}>
        ...
        <section className='flex gap-5 items-center'>
          <button onClick={() => setShowCart(true)} className='relative group'>
            ...
            <span className='absolute flex items-center justify-center -top-2 -right-2 bg-shop-500 w-5 h-5 text-white rounded-full text-sm font-bold group-hover:bg-red-500 transition-colors'>4</span>
          </button>
